...
subroutine dump_density (II)
	...
	do i = -NX/2, NX/2
		do j = -NY/2, NY/2
			...
			dble(i*DSPACE),dble(j*DSPACE),dble(GRID(i,j)*CONJG(GRID(i,j))),SQRT((velx(i,j)**2.0d0)+(vely(i,j)**2.0d0))
		end do
		write (unit=7,fmt="(a)") " "
	end do
	close(7)
...
subroutine dump_wavefunction (II)
	...
	do i = -NX/2, NX/2
		do j = -NY/2, NY/2
			...
			dble(i*DSPACE),dble(j*DSPACE),dble(GRID(i,j)),aimag(GRID(i,j)),DBLE(OBJPOT(i,j))
		end do
		write (unit=7,fmt="(a)") " "
	end do
	close(7)
...
subroutine dump_vortex_locations (II)
	...
	write(fname, '(i0,a,i0.4)') LOOPNO,'plotvort.',II/dumpv
	open (7, FILE = fname)
	do k=0,vort-1
